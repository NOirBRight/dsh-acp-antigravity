window.__ModuleLoader__.load({
	...
	factory: (require) => {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							row.authorizationUrl ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("a", {
		...
		function ExternalAgentsSection(props) {
			...
			const loginWindow = (0, react.useRef)(null);
			const refresh = async () => {
				let next = await load();
				setSnapshot(next);
			...
			(0, react.useEffect)(() => {
				...
				const timer = window.setInterval(() => {
					load().then((next) => {
						...
						if (url !== void 0 && loginWindow.current !== null && !loginWindow.current.closed) try {
							loginWindow.current.location.href = url;
						} catch {}
					}).catch(() => void 0);
				}, 500);
				return () => window.clearInterval(timer);
