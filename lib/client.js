window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function groupNativeActivity(rows, observations = [], texts = []) {
			const agents = /* @__PURE__ */ new Map();
			...
			const points = [
				...observations,
				...rows.flatMap((row) => row.state.ownership === void 0 ? [] : [{
					...row,
				...
				...texts.map((text) => ({
					...
					ownership: {
						trajectoryId: text.trajectoryId,
						...text.parentTrajectoryId === void 0 ? {} : { parentTrajectoryId: text.parentTrajectoryId }
					}
				}))
			];
			...
			for (const text of texts) {
				...
				if (agent === void 0) roots.push(branch);
					...
					if (order < agent.order) {
						agent.order = order;
						agent.firstSeenAt = text.firstSeenAt;
					}
				}
			}
  ...
  pointer-events: none;
		...
		*/
		...
		function formatQuotaDetail(detail) {
			...
			if (!/^\d{4}-\d{2}-\d{2}T/.test(detail)) return detail;
			const ms = Date.parse(detail);
			if (!Number.isFinite(ms)) return detail;
			const days = Math.max(0, Math.ceil((ms - Date.now()) / 864e5));
			const date = new Date(ms).toLocaleDateString(void 0, {
				month: "short",
				day: "numeric"
			});
			return typeof navigator !== "undefined" && String(navigator.language || "").startsWith("zh") ? `重置时间：${date}（还剩 ${days} 天）` : `Usage limits reset on ${date} (${days} days left)`;
		}
