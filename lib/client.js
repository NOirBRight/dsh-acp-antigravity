window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ACP_SETTINGS_RPC_METHOD = "plugin-rpc/antigravity";
		...
		function callAcpSettingsRpc(rpc, endpoint, payload, signal) {
			return rpc.call("/api", ACP_SETTINGS_RPC_METHOD, {
				endpoint,
				payload
			}, signal);
		}
		...
		function decodeInstallProgress(value) {
			if (!isRecord$1(value) || typeof value.phase !== "string" || typeof value.message !== "string") return void 0;
			if (typeof value.downloadedBytes !== "number" || typeof value.totalBytes !== "number") return void 0;
			return {
				phase: value.phase,
		...
		function decodeCatalogModel(value) {
			...
			const inputTokenLimit = typeof value.inputTokenLimit === "number" && Number.isSafeInteger(value.inputTokenLimit) && value.inputTokenLimit > 0 ? value.inputTokenLimit : void 0;
			...
			return {
				...
				...isRecord$1(value.effortMap) ? { effortMap: Object.fromEntries(Object.entries(value.effortMap).filter((entry) => typeof entry[1] === "string")) } : {},
				...vision === void 0 ? {} : { vision },
				...thinking === void 0 ? {} : { thinking },
				...inputTokenLimit === void 0 ? {} : { inputTokenLimit },
		...
		function decodeAuthAttempt(value) {
			if (!isRecord$1(value) || value.status !== "pending" && value.status !== "failed" && value.status !== "expired") return void 0;
			return {
				status: value.status,
			...
			for (const row of value.rows) {
				...
				rows.push({
					...
					...(() => {
						...
						return attempt === void 0 ? {} : { authAttempt: attempt };
					})(),
		...
		function decodeCatalogPersistence(value) {
			...
			const raw = Array.isArray(value.catalogOverlay) ? value.catalogOverlay : void 0;
		...
		*/
		function foldAntigravityToolEvent(state, event) {
			if (event.type !== "antigravity/tool-start" && event.type !== "antigravity/tool-update") throw new Error("Antigravity activity is not a tool row event");
		...
		//#endregion
		//#region src/request-telemetry.ts
		...
		*/
		function decodeUsageSnapshots(value) {
			if (!isRecord(value) || value.version !== 1 || value.provenance !== "sdk-cumulative" || value.baseline !== "fresh-session" && value.baseline !== "cumulative" || typeof value.completed !== "boolean") return void 0;
			const promptId = stringValue(value.promptId);
			const sessionKey = stringValue(value.sessionKey);
			if (promptId === void 0 || sessionKey === void 0) return void 0;
			const snapshots = [];
			for (const source of [value.start, value.end]) {
				...
				for (const key of [
					...
					if (count !== null && (typeof count !== "number" || !Number.isSafeInteger(count) || count < 0)) return void 0;
					snapshot[key] = count;
				}
		...
		*/
		function decodeRequestTelemetry(value) {
			if (!isRecord(value) || value.version !== 1 || value.provenance !== "ccpa-proxy" || value.scope !== "session") return void 0;
			const promptId = stringValue(value.promptId);
			const sessionKey = stringValue(value.sessionKey);
			if (promptId === void 0 || sessionKey === void 0 || !Array.isArray(value.requests)) return void 0;
			const requests = [];
			...
			for (const raw of value.requests) {
				...
				if (requestId === void 0 || model === void 0 || ids.has(requestId)) return void 0;
				if (raw.status !== "completed" && raw.status !== "failed" && raw.status !== "cancelled") return void 0;
				if (typeof raw.durationMs !== "number" || !Number.isFinite(raw.durationMs) || raw.durationMs < 0) return void 0;
				const usageMetadata = raw.usageMetadata;
				if (usageMetadata !== null && !isRecord(usageMetadata)) return void 0;
				if (usageMetadata !== null && countKeys.some((key) => usageMetadata[key] !== void 0 && (typeof usageMetadata[key] !== "number" || !Number.isSafeInteger(usageMetadata[key]) || usageMetadata[key] < 0))) return void 0;
				ids.add(requestId);
				requests.push({
					requestId,
					model,
		...
		const ACTIVITY_READ_AFTER_ENDPOINT = "activity/read-after";
		...
		*/
		function decodeActivityPage(value, afterSeq) {
			if (!isRecord(value)) throw corrupt("page is not an object");
			if (value.version !== 1) throw corrupt("page has an unknown version");
			...
			if (last === void 0 ? nextCursor !== afterSeq : last.seq !== nextCursor) throw corrupt("page cursor does not match its records");
			return {
				version: 1,
				records,
				nextCursor,
		...
		function decodeRecordValue(value, seq) {
			...
			if (type === "antigravity/usage-snapshots") {
			...
			if (type === "antigravity/request-telemetry") {
			...
			if (type === "antigravity/agent-text" && isAgentTextData(value.data)) return {
				seq,
				time,
				type,
				data: value.data
			};
			if (type === "antigravity/user-question-answer" && isUserQuestionAnswerData(value.data)) return {
				seq,
				time,
				type,
				data: value.data
		...
		//#endregion
		//#region node_modules/.pnpm/@deepseek-ai+dsh-acp-provider@https+++github.com+NOirBRight+dsh-acp-provider+releases+d_96afdc1d850e625c51a1fbf806ea8fcf/node_modules/@deepseek-ai/dsh-acp-provider/dist/native-history.js
		...
		*/
		var StaleNativeHistoryCursorError = class extends Error {
		...
		*/
		...
		function entryFor(scope, sessionId, adapter) {
		...
		function resetRetainedFold(entry) {
		...
		*/
		async function followNativeHistory(entry, controller) {
			let changed = false;
		...
		async function pollNativeHistory(entry) {
		...
		function scheduleNativeHistory(entry) {
			if (entry.listeners.size === 0) return;
			if (entry.timer !== void 0) return;
			entry.timer = setTimeout(() => {
				entry.timer = void 0;
				pollNativeHistory(entry);
		...
		*/
		function getNativeHistoryStore$1(scope, sessionId, adapter) {
			const entry = entryFor(scope, sessionId, adapter);
			return {
				subscribe: (listener) => {
					entry.listeners.add(listener);
					if (entry.listeners.size === 1) pollNativeHistory(entry);
				...
				refresh: () => {
					...
					entry.foldEstablished = false;
					if (entry.listeners.size > 0) pollNativeHistory(entry);
				}
			};
		}
		//#endregion
		...
		*/
		function createNativeActivityFoldState() {
			return {
				rows: [],
		...
		*/
		function applyActivityRecords(state, records) {
			for (const record of records) {
				if (record.type === "antigravity/session-ready") {
					state.epoch += 1;
					continue;
				}
				if (record.type === "antigravity/agent-observed") {
					const key = `${state.epoch}\n${record.data.trajectoryId}`;
					if (!state.agents.has(key)) state.agents.set(key, {
				...
				if (record.type === "antigravity/agent-text") {
					const last = state.texts.at(-1);
					if (last !== void 0 && last.epoch === state.epoch && last.kind === record.data.kind && last.trajectoryId === record.data.trajectoryId && last.parentTrajectoryId === record.data.parentTrajectoryId) {
				...
				if (record.type === "antigravity/user-question-answer" || record.type === "antigravity/full-access-authorized" || record.type === "antigravity/request-telemetry" || record.type === "antigravity/usage-snapshots") continue;
		...
		*/
		...
		async function loadActivityPage(rpc, sessionId, afterSeq, signal) {
			const result = await callAcpSettingsRpc(rpc, ACTIVITY_READ_AFTER_ENDPOINT, {
				sessionId,
				afterSeq
			}, signal);
			if (!result.ok) {
				const message = result.error?.message ?? "Antigravity activity history is unavailable";
				if (result.error?.code === "stale-cursor") throw new StaleNativeHistoryCursorError(message);
				throw new Error(message);
			}
			return decodeActivityPage(result.value, afterSeq);
		}
		function snapshotOf(state, error) {
			return {
		...
		function getNativeHistoryStore(rpc, sessionId) {
			return getNativeHistoryStore$1(rpc, sessionId, {
		...
		*/
		...
		//#endregion
		//#region node_modules/.pnpm/@deepseek-ai+dsh-acp-provider@https+++github.com+NOirBRight+dsh-acp-provider+releases+d_96afdc1d850e625c51a1fbf806ea8fcf/node_modules/@deepseek-ai/dsh-acp-provider/dist/native-ui.js
		const TOOL_PRESENTATION = {
			read: {
				titleKey: "tool.title.read",
				icon: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconBrowseOutlineRegular, { size: 14 })
			},
			todo_write: {
				titleKey: "todo.rowTitle",
				icon: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconChecklistOutlineRegular, { size: 14 })
			},
			bash: {
				titleKey: "tool.title.bash",
				icon: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconApiOutlineRegular, { size: 14 })
			},
			grep: {
				titleKey: "tool.title.grep",
				icon: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconSearchOutlineRegular, { size: 14 })
			},
			glob: {
				titleKey: "tool.title.glob",
				icon: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconSearchOutlineRegular, { size: 14 })
			},
			web_search: {
				titleKey: "tool.title.webSearch",
				icon: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconSearchOutlineRegular, { size: 14 })
			},
			web_fetch: {
				titleKey: "tool.title.webFetch",
				icon: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconGlobeOutlineRegular, { size: 14 })
			},
			write: {
				titleKey: "tool.title.write",
				icon: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconEditOutlineRegular, { size: 14 })
			},
			edit: {
				titleKey: "tool.title.edit",
				icon: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconEditOutlineRegular, { size: 14 })
			}
		};
		const sep = " · ";
		...
		function NativeToolCard(props) {
			...
			const expandable = props.input !== void 0 || props.output !== void 0 || props.detail !== void 0;
			return (0, react_jsx_runtime.jsx)("div", {
				"data-native-tool-card": props.callId,
				"data-state": props.state,
				title: renamed ? props.nativeName : title,
				"aria-label": title + (props.summary === "" ? "" : sep + props.summary),
				style: {
					width: "100%",
					minWidth: 0
				},
				children: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.DisclosureRow, {
					...
					collapsedContent: props.summary === "" ? null : (0, react_jsx_runtime.jsx)("span", {
						"data-card-summary": props.summary,
						style: {
							minWidth: 0,
							overflow: "hidden",
					...
					children: (0, react_jsx_runtime.jsx)("div", {
						...
						children: (0, react_jsx_runtime.jsx)(NativeToolBody, { ...props })
					})
				})
			});
		}
		function titleOf(toolName, t) {
			return t(TOOL_PRESENTATION[toolName]?.titleKey ?? "tool.title.generic");
		}
		function iconOf(toolName) {
			return TOOL_PRESENTATION[toolName]?.icon ?? (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconSparkleRegular, { size: 14 });
		}
		function detailLabels(kind, t) {
			return {
				copy: t("copy"),
				copied: t("copied"),
				collapse: t("collapse"),
				collapseAria: t(`${kind}.collapseAria`),
				expandAria: (count) => t(`${kind}.expandAria`, kind === "terminal" ? { n: count } : { count }),
				expand: (count) => t(`${kind}.expandRest`, kind === "terminal" ? { n: count } : { count })
			};
		}
		...
		function NativeToolBody(props) {
			const [inspect, setInspect] = (0, react.useState)(false);
			const io = (0, react_jsx_runtime.jsx)(NativeToolIO, {
				input: props.input,
				output: props.output,
				state: props.state,
				t: props.t
			});
			if (props.detail === void 0) return io;
			let body;
			switch (props.detail.kind) {
				case "read":
					body = (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.ReadBlock, {
						label: props.detail.label,
						lines: props.detail.lines,
						totalLines: props.detail.totalLines,
						maxLines: 8,
						labels: {
							...detailLabels("read", props.t),
							...codeToolbarLabels(props.t),
							window: (shown, total) => props.t("read.window", {
								shown,
								total
							})
						}
					...
					break;
				case "diff":
					body = (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.DiffBlock, {
						diffs: [...props.detail.diffs],
						maxLines: 9,
						labels: {
							...detailLabels("diff", props.t),
							...codeToolbarLabels(props.t)
						}
					});
					break;
				case "terminal":
					body = (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.TerminalBlock, {
						command: props.detail.command,
						output: props.detail.output,
						running: props.state === "running",
						cwd: props.detail.cwd,
						exitCode: props.detail.exitCode,
						labels: {
							...detailLabels("terminal", props.t),
							signal: (signal) => props.t("terminal.signal", { signal }),
							exitCode: (code) => props.t("terminal.exitCode", { code }),
							noExitCode: props.t("terminal.noExitCode"),
							running: props.t("terminal.running"),
							failed: props.t("terminal.failed"),
							done: props.t("terminal.done"),
							noOutput: props.t("terminal.noOutput")
						}
					});
					break;
				case "empty": body = (0, react_jsx_runtime.jsx)("div", {
					"data-card-empty-result": true,
					style: {
						color: "var(--dsw-alias-label-tertiary)",
						fontSize: 13
					},
					children: props.t("terminal.noOutput")
				});
			}
		...
		function prettyNativeToolPayload(text) {
			const value = parse$1(text);
			return typeof value === "string" ? value : JSON.stringify(value, null, 2);
		}
		function NativeToolIO({ input, output, state, t }) {
			...
			return (0, react_jsx_runtime.jsx)("div", {
				...
				children: [["row.input", input], ["row.output", output]].map(([label, text]) => text === void 0 ? null : (0, react_jsx_runtime.jsxs)("div", {
					"data-card-result": label === "row.output" ? state : void 0,
					style: {
						...ioSection,
						borderTop: label === "row.output" && input !== void 0 ? "1px solid var(--dsw-alias-border-l2)" : void 0
					},
					children: [
						...
						(0, react_jsx_runtime.jsx)("pre", {
							style: {
								...ioText,
								color: label === "row.output" && state === "error" ? "var(--dsw-alias-state-error-primary)" : void 0
							},
							children: prettyNativeToolPayload(text)
						}),
						(0, react_jsx_runtime.jsx)("button", {
							type: "button",
							"data-native-copy": true,
							"aria-label": `${t("copy")} ${t(label)}`,
							style: {
							...
							onClick: () => {
								(0, _deepseek_ai_dsh_client_ui_primitives.writeClipboard)(prettyNativeToolPayload(text)).then((ok) => {
									if (ok) setCopied(text);
								});
							},
							children: copied === text ? t("copied") : t("copy")
						})
					]
				}, label))
			});
		...
		function nativeToolName(name, input) {
			...
			if (parsed !== void 0 && Object.entries(parsed).some(([key, value]) => (ARG_KEY_ALIASES[key] ?? key) === "command" && typeof value === "string")) return "bash";
			return name;
		}
		...
		function nativeToolSummary(state, toolName) {
			...
			if (args !== void 0) for (const key of SUMMARY_KEYS) {
		...
		function nativeToolCardModel(state) {
			const toolName = nativeToolName(state.name, state.input);
			const input = nativeToolArgs(state);
			const output = state.status === "completed" || state.status === "failed" ? state.status === "failed" ? state.error ?? state.output ?? "" : state.output ?? state.error ?? "" : void 0;
		...
		function detailOf(toolName, argsRaw, result) {
			const input = parseRecord(argsRaw) ?? {};
			const output = result === void 0 ? void 0 : parse(result);
			const text = typeof output === "string" && output === result && (toolName === "read" ? /^\s*[\[{]/u : /^\s*(?:\{|\[\s*[\[{"])/u).test(output) ? void 0 : outputText(output);
			...
			if (toolName === "read" && path !== void 0 && text !== void 0) {
				...
				return { detail: {
					...
					label: path,
					lines,
					totalLines: typeof total === "number" && Number.isSafeInteger(total) && total >= offset + lines.length - 1 ? total : lines.length
				} };
			}
			if (toolName === "edit" || toolName === "write") {
				...
				if (Array.isArray(output)) for (const item of output) {
					...
					if (diff.type !== "diff" || typeof diff.path !== "string" || typeof diff.newText !== "string" || !(diff.oldText === null || typeof diff.oldText === "string")) {
						diffs.length = 0;
						break;
					}
					...
					diffs.push({
						path: diff.path,
						oldText: created ? null : diff.oldText,
						newText: created ? diff.newText.slice(header.length + 1) : diff.newText
					});
				}
				if (diffs.length === 0 && path !== void 0) {
					if (toolName === "write" && typeof input.content === "string") diffs.push({
						path,
						oldText: null,
						newText: input.content
					});
					if (toolName === "edit" && typeof input.old_string === "string" && typeof input.new_string === "string") diffs.push({
						path,
						oldText: input.old_string,
						newText: input.new_string
					});
				}
				if (diffs.length > 0) return { detail: {
					kind: "diff",
					diffs
				} };
			}
			...
			const terminalOutput = text ?? (typeof shell.stdout === "string" || typeof shell.stderr === "string" ? [shell.stdout, shell.stderr].filter((part) => typeof part === "string").join("") : void 0);
			if (toolName === "bash" && typeof command === "string" && (result === void 0 || terminalOutput !== void 0)) {
		...
		function outputText(value) {
			...
			if (Array.isArray(value)) {
				...
				if (parts.every((part) => part !== void 0)) return parts.join("\n");
			}
		}
		//#endregion
		//#region src/web/AntigravityToolNode.tsx
		function AntigravityToolNode({ row, conversationT }) {
			const translate = conversationT;
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(NativeToolCard, {
				...nativeToolCardModel(row.state),
				t: translate
			});
		}
		//#endregion
			...
			switch (branch.kind) {
				...
				case "text": return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					...
					style: {
						...
						color: branch.thought ? "var(--dsw-alias-label-tertiary)" : "var(--dsw-alias-label-primary)"
					},
		...
		*/
		function groupNativeActivity(rows, _observations = [], texts = []) {
			const roots = [];
			for (const row of rows) roots.push({
				kind: "tool",
				key: row.key,
				row,
				order: Number(row.key)
			});
			for (const text of texts) roots.push({
				kind: "text",
				key: text.key,
				text: text.text,
		...
		*/
		...
		function NativeTurnContainer(props) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				...
				children: [
					...
					branches.map((branch) => {
						const unattributed = !isOwnedByTurn(Date.parse("firstSeenAt" in branch ? branch.firstSeenAt : branch.row.firstSeenAt), startMs, endMs);
		...
		//#endregion
		//#region node_modules/.pnpm/dsh-llm-providers-ui@https+++github.com+NOirBRight+dsh-llm-providers-ui+releases+downlo_a6ae55fcce481de6e7e2f191b98073c7/node_modules/dsh-llm-providers-ui/lib/model-catalog.js
		const inputStyle = {
		...
		*/
		...
		//#endregion
		//#region node_modules/.pnpm/dsh-llm-providers-ui@https+++github.com+NOirBRight+dsh-llm-providers-ui+releases+downlo_a6ae55fcce481de6e7e2f191b98073c7/node_modules/dsh-llm-providers-ui/lib/provider-ui.js
		...
		function pickPrimaryWindow(windows) {
			...
			if (best !== void 0 && best.remainingPercent === 100 && !nonEmptyString$1(best.resetsAt) && periodRank(best) < MONTHLY_PERIOD_RANK) {
				let fallback;
				for (const quotaWindow of windows) {
		...
		*/
		...
		//#endregion
		//#region node_modules/.pnpm/dsh-llm-providers-ui@https+++github.com+NOirBRight+dsh-llm-providers-ui+releases+downlo_a6ae55fcce481de6e7e2f191b98073c7/node_modules/dsh-llm-providers-ui/lib/usage-readers.js
		/** Plain-object guard shared by the reader factories and the sidebar cache validator. */
		...
		*/
		...
		function createAntigravityUsageReader() {
			return {
				...
				async read(rpc, _refresh, signal) {
					...
					const hit = quota.groups.flatMap((group) => group.buckets.map((bucket) => ({
					...
					const remaining = headlineRemainingPercent(hit.bucket.remainingFraction);
					...
					const resetsAt = hit.bucket.resetTime === void 0 ? void 0 : resetsPrefix + new Date(hit.bucket.resetTime).toLocaleString();
					return {
						status: "ready",
						fetchedAt: quota.observedAt,
						windows: [{
		...
		function antigravityAccessKind(hostname, userAgent = "") {
		...
		function antigravityAccessHintKey(kind) {
		...
		*/
		...
		function CallbackPaste({ t, disabled, onSubmit }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						style: muted,
						children: t("pasteCallback")
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
						style: field,
						children: [t("callbackUrl"), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
							style: control,
							value,
							autoComplete: "off",
							spellCheck: false,
		...
		function formatInstallPercent(downloadedBytes, totalBytes) {
			if (totalBytes <= 0) return "0%";
			const pct = 100 * downloadedBytes / totalBytes;
			if (downloadedBytes > 0 && pct < .1) return "<0.1%";
		...
		*/
		function AntigravityCardBody({ t, row, snapshot, state, quota, quotaError, quotaLoading, working, polling, saving, dirty, onAction, onRefresh, onRefreshModels, onRefreshQuota, onCatalogChange, onPersist, onDiscard, accessKind, mode, detailCopy, sharedTemplate, sharedUsage, onSharedQuotaRefresh }) {
			...
			const pickerSections = [{
				...
				models: (candidates ?? row.models).map((model) => ({
					id: model.id,
					name: model.name,
					...model.reasoning !== void 0 && model.reasoning.efforts.length > 0 ? { hint: t("thinking") } : {}
				}))
			}];
			const adoptModels = () => {
				...
				for (const id of picked) {
					...
					selected.push(previous === void 0 ? candidate : {
						...previous,
						...candidate
					});
				}
			...
			const installActions = /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
				style: {
					display: "inline-flex",
					gap: 8
				},
			...
			const installStatus = /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
				style: {
					...muted,
					margin: 0
				},
				...
				style: { paddingLeft: 0 },
				children: [snapshot.install.message, snapshot.install.totalBytes > 0 ? " " + formatInstallPercent(snapshot.install.downloadedBytes, snapshot.install.totalBytes) : ""]
			}) : null] });
			const installBlock = /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "c-control",
				children: [installStatus, /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					style: {
						display: "flex",
						gap: 8,
						marginTop: 6
					},
					children: installActions
				})]
			});
			const accountActions = /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				style: {
					...actions,
					justifyContent: "space-between"
				},
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", {
					...
					style: muted,
					children: row.authenticated ? t("connected") : state === "missing" ? t("missingBadge") : state === "error" ? t("errorBadge") : t("authBadge")
				})] }), row.authenticated ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
					style: {
						display: "inline-flex",
						gap: 8
					},
				...
				}) : state !== "missing" && !loginActive ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					type: "button",
					style: button,
					disabled: working || polling,
			...
			const accountBody = /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				...
				loginActive && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							style: muted,
							children: t(antigravityAccessHintKey(kind))
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							style: muted,
							children: row.authAttempt?.status === "failed" || row.authAttempt?.status === "expired" ? row.authAttempt.message ?? t("failed") : t("loginWaiting")
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							style: actions,
							children: [
								loginUrl && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("a", {
									href: loginUrl,
									target: "_blank",
								...
								snapshot.signingIn || row.authAttempt?.status === "pending" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									style: button,
									disabled: working,
						...
						(loginUrl || row.authAttempt) && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CallbackPaste, {
							t,
							disabled: working,
							onSubmit: (url) => onAction("complete-login", url)
						})
					]
				}),
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							...
							onClick: () => {
								...
								setConfirm(void 0);
								setMenu(false);
								onAction(action === "switch" ? "sign-in" : "sign-out");
							},
			...
			if (mode === "detail" && sharedTemplate !== void 0 && detailCopy !== void 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)(sharedTemplate, {
					name: "Antigravity",
					role: "agent",
					...
					account: {
						...
						body: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [showInstall ? installStatus : null, accountBody] })
					},
					quota: {
		...
		*/
		function ExternalAgentsSection({ t, load, save, run, quota: readQuota, ...slot }) {
			...
			const seedInstallProgress = (value) => {
				const install = decodeInstallProgress(value);
				const current = snapshotRef.current;
				if (install === void 0 || current === void 0) return;
				accept({
					...current,
					install
				});
			...
			(0, react.useEffect)(() => {
				...
				const tick = () => {
					...
					pending = true;
					load().then((next) => {
						if (!stopped && mounted.current) accept(next);
					}).catch(fail).finally(() => {
						pending = false;
					});
				};
			...
			if (slot.mode === "detail" && row && snapshot) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AntigravityCardBody, {
				t,
				row,
				snapshot,
				state,
		...
		function apply(ctx) {
			...
			const publishAccount = (state) => {
				...
				account.state = state;
				ctx.get("providerDirectory")?.update?.("antigravity");
			};
			installProviderDirectory(ctx, () => acceptedRow?.models.length, {
				account: () => ({ state: account.state }),
				binding: {
					channel: ACP_SETTINGS_RPC_METHOD,
					endpoint: ACTIVITY_BINDING_ENDPOINT
				}
			});
			const load = async () => {
				const result = await callAcpSettingsRpc(rpc, SNAPSHOT_ENDPOINT, {}, void 0);
			...
			const run = async (action, value) => {
				const result = await callAcpSettingsRpc(rpc, "run", {
					action,
					...value === void 0 ? {} : { value }
				}, void 0);
				if (!result.ok) throw new Error(result.error.message);
			...
			const pick = async () => {
				const result = await callAcpSettingsRpc(rpc, PICK_ENDPOINT, {}, void 0);
				if (!result.ok) throw new Error(result.error.message);
			...
			}, "dsh-acp-antigravity: account snapshot");
