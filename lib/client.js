window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#endregion
		//#region src/web/catalog-overlay.ts
		/** Overlay ready External Agent groups onto a ModelDirectory instance. */
		function mergeCatalogGroups(base, extra) {
		...
		function overlayDirectory(inner, fetchCatalog) {
		...
		async function fetchExternalCatalog(rpc) {
			try {
				const result = await rpc.call(ACP_SETTINGS_RPC_CHANNEL, CATALOG_ENDPOINT, {}, void 0);
		...
		function apply(ctx) {
			...
			if (typeof ctx.inject === "function") ctx.inject(["modelDirectories"], (scope) => {
				...
				scope.modelDirectories.directoryFor = (id) => {
					const inner = orig(id);
