...
export class AntigravityProvider implements ExternalAgentProvider {
  ...
  async openSession(request: ExternalAgentOpenRequest): Promise<ExternalAgentSession> {
    ...
    if (request.permissionMode === 'full-access') {
      if (request.fullAccessConfirmed !== true) throw new Error('Antigravity full-access requires explicit confirmation')
      if (this.dependencies.auditFullAccess === undefined) throw new Error('Antigravity full-access requires an audit callback')
      await this.dependencies.auditFullAccess({ provider: String(this.info.id), instanceId: this.config.instanceId, session: String(request.session), ...(request.fullAccessAuditId === undefined ? {} : { auditId: request.fullAccessAuditId }), mode: 'full-access' })
    }
  ...
  /** Validate the configured executable pair without starting a process. */
  async validateInstallation(): Promise<Awaited<ReturnType<typeof validateAntigravityInstallation>>> {
    ...
    if ('status' in result) this.status = { status: result.status, profileDirectory: this.config.stateDirectory, message: result.message }
    else {
      const model = this.status.model
      this.status = { status: this.status.status === 'ready' ? 'ready' : 'authentication-required', profileDirectory: this.config.stateDirectory, ...(result.version === undefined ? {} : { version: result.version }), ...(model === undefined ? {} : { model }) }
    }
    return result
  }
  ...
  private async openConnection(cwd: string, signal?: AbortSignal): Promise<AcpConnection> {
    ...
    try {
      await this.authenticateIfConfigured(connection, signal)
      return connection
    } catch (error) {
      this.connections.delete(connection)
      await connection.close()
...
export class AntigravitySession implements ExternalAgentSession {
  ...
  async runTurn(request: ExternalAgentTurnRequest, host: ExternalAgentTurnHost): Promise<ExternalAgentTurnResult> {
    ...
    this.connection.setNotificationHandler((method, params) => {
      ...
      try {
        const event = normalizeAntigravitySessionUpdate(update, { maxTextBytes: this.config.maxEventTextBytes ?? 1024 * 1024, maxPayloadBytes: this.config.maxEventPayloadBytes ?? 16 * 1024 * 1024 })
        if (event === null) return
        if (event.type === 'assistant-delta') text += event.text
        events = events.then(() => host.publish(event))
      } catch {
        events = events.then(async () => {
          try { await host.publish({ type: 'notice', level: 'warning', message: 'Antigravity ignored a malformed session update.' }) } catch { /* The turn host may already be expired. */ }
        })
      }
    })
    const onAbort = () => {
    ...
    try {
      ...
      const status = request.signal.aborted || stopReason === 'cancelled' ? 'cancelled' : stopReason === 'refusal' || stopReason === 'error' ? 'failed' : 'completed'
      await host.publish({ type: 'turn-result', status, content: text })
      return { status, text, nativeSessionId: this.nativeSession, ...(this.ref.resumeCursor === undefined ? {} : { resumeCursor: this.ref.resumeCursor }), ...(status === 'failed' ? { error: String(stopReason ?? 'provider turn failed') } : {}) }
    } catch (error) {
