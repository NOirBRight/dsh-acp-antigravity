...
import { shouldMountDshRuntime } from '../src/compatibility.js'

const VERIFIED = new Set(['0.1.5-rc.1', '0.1.5-rc.2'])
...
describe('DSH forward compatibility policy', () => {
  ...
  it('does not warn for a verified runtime', () => {
    const warnings: string[] = []
    expect(shouldMountDshRuntime(logger(warnings), 'test-plugin', '0.1.5-rc.2', VERIFIED)).toBe(true)
    expect(warnings).toEqual([])
  })

  it('declares the exact supported published DSH release policy', () => {
